...
WshShell.Run "wsl -d trace-vm -u root -- bash -c ""pkill -9 -f supermemory-server 2>/dev/null""", 0, True
WScript.Sleep 500

' Start Supermemory silently in WSL
wslLogPath = Replace(smt, "\", "/")
wslLogPath = "/mnt/c" & Mid(wslLogPath, 3)
WshShell.Run "wsl -d trace-vm -u root bash -c ""cd /root && export SUPERMEMORY_NO_PROMPT=1 && export OPENAI_API_KEY=dummy && /root/.supermemory/bin/supermemory-server 2>&1 | tee " & wslLogPath & "/supermemory.log""", 0, False
